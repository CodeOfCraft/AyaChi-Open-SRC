	...
	if (_renderType != RenderType::None)
	{
		if (_renderType >= RenderType::D3D9 && _renderType <= RenderType::D3D12)
		{
			...
			else if (_renderType == RenderType::D3D11)
			{
				...
				// 11.0 优先：Latite 同款顺序。10_1 在前的话 dummy 可能建在低特性级别上，
				// 与游戏交换链的实现类不再保证一致
				...
				swapChainDesc.SampleDesc = sampleDesc;
				swapChainDesc.BufferUsage = DXGI_USAGE_RENDER_TARGET_OUTPUT;
				// dummy 必须用 flip 模型：老式 DISCARD 和 flip 是 dxgi.dll 里两个不同的类，
				// 虚表槽位指向不同的 Present 实现。从老式 dummy 上钩到的 Present，
				// 游戏的 flip/D3D12 交换链永远不会调用（Latite 同样用 FLIP_DISCARD + 2 缓冲）
				...
				// 交换链虚表拷 44 项：除了 [8]Present/[13]ResizeBuffers，
				// 还要覆盖 [39]ResizeBuffers1（D3D12 的窗口缩放走这个入口）
				g_methodsTable = (uint150_t*)::calloc(256, sizeof(uint150_t));
				::memcpy(g_methodsTable, *(uint150_t**)swapChain, 44 * sizeof(uint150_t));
				::memcpy(g_methodsTable + 44, *(uint150_t**)device, 43 * sizeof(uint150_t));
				::memcpy(g_methodsTable + 44 + 43, *(uint150_t**)context, 144 * sizeof(uint150_t));
			...
			else if (_renderType == RenderType::D3D12)
			{
				...
				g_methodsTable = (uint150_t*)::calloc(200, sizeof(uint150_t));
				...
				::memcpy(g_methodsTable + 44 + 19 + 9 + 60, *(uint150_t**)swapChain, 44 * sizeof(uint150_t));
